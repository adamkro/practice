...
def login():
    session.clear() #clear previous session data. not sure if it wokrs.
    name = request.form.get("name")
    password = request.form.get("password")
    if name is None:
        return render_template("login.html")
    elif User.query.filter_by(name=name).first().password == password: #if login is successful
...
def book_page(book_id):
    ...
    if len(book.isbn) == 9: #CSV file was bad, need to correct certain ISBN's
    ...
    if res.status_code == 200:    #if API request is successful, update data
        data = res.json()
...
def add_review(book_id):
    #Adds a review then redirects to book_page
    book = Book.query.get(book_id)
